...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    nickname = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Никнейм"
    )
    ...
    phone_number = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Телефон", unique=True
    )
    email = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Почта"
    )
    fcm_token = models.CharField(max_length=500, null=True, blank=True)
    consecutive_meditation_days = models.IntegerField(
        default=0, verbose_name="Дни непрерывной медитации"
    )
...
class CodePhone(models.Model):
    ...
    is_confirmed = models.BooleanField(
        max_length=100, blank=True, null=True, default=False
    )
